 ...
 /* 	const { navigate } = this.props.navigation;*/
    ...
    outputRange: [0, 100]
  	...
  	for (let i=0; i < this.state.countries.length; i++) {
  		if(this.state.countries[i].europe && !this.state.countries[i].schengen){
  			this.state.countries[i].color = 'green'
  		} else if (this.state.countries[i].europe && this.state.countries[i].schengen){
  			this.state.countries[i].color = 'red'
  		}
  		if(this.state.countries[i].europe) {

  			ctryList.push(
  				<Animated.View key= {i} style={[{flex: 1, flexDirection: 'row'}, {marginLeft}]}>
  				<View style={{flex: .4}}>
  					<Text style={{fontSize: 22, color: this.state.countries[i].color}}>{this.state.countries[i].name}</Text>
  				</View>
  				<View style={{flex: .2}}>
  					<Image style={{width: 28,height: 28}} source={this.state.countries[i].flag} />
  				</View>
  				</Animated.View>
  				)
  		}
  	...
  	return (
  		...
  		<View>
  	<TouchableOpacity onPress={() => navigate('App')}><Icon name="ios-arrow-round-back-outline" size={40} color='white' /></TouchableOpacity>
  </View>
  <View>
  		<ScrollView style={{height: 600}}>
  			{ctryList}
  		</ScrollView>
  			<Text style={{fontSize: 14, color: 'white'}}> hello</Text>
  		</View>
  		</View>
